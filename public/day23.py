...
class State:
    ...
    def get_legal_moves(self,force_src=-1):
        ...
        for src in self.tiles:
            #print("S")
            src_top_value=src.get_top_value()
            if src_top_value is None:
            ...
            for dst in self.tiles:
                ...
                if dst.type!='W' and src_top_value!=dst.type:
                    continue#Rule, this type of animal doesn't live in that destination-bucket
#                if src.type!='W' and src_top_value=="D" and src.position!=9 and (dst.position>9 or dst.position<src.position):
#                    continue
                if dst.get_bot_empty() is None:
                    continue#Rule-destination tile is full
                if dst.type!='W' :
                    dst_values=set(dst.values)
                    dst_values.discard(src_top_value)
                    dst_values.discard(".")
                    if len(dst_values)!=0:
                ...
                new_state.tiles[src.position-1].values[src.get_move_tile_src_index()]="."
                new_state.tiles[dst.position-1].values[dst.get_move_tile_dst_index()]=src_top_value
                #print(new_state.to_string())
                res.append((cost, new_state.to_string(),dst.type))
        for cand in res:
...
do_all_moves(init_state.to_string())
